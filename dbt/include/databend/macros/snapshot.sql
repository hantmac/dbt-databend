{% macro databend__snapshot_hash_arguments(args) -%}
...
{% macro databend__snapshot_string_as_time(timestamp) -%}
   {%- set result = "to_timestamp('" ~ timestamp ~ "')" -%}
   {{ return(result) }}
{%- endmacro %}


{% macro databend__post_snapshot(staging_relation) %}
...
{% macro build_snapshot_staging_table(strategy, sql, target_relation) %}
    ...
    {% set select = snapshot_staging_table(strategy, sql, target_relation) %}
               ...
               {% macro databend__snapshot_merge_sql(target, source, insert_cols) -%}
